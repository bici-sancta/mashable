...
# ...	use cx_scale = 3 for pdf landscape mode knitted docs
	...
	plot(mash$shares, col = "darkgoldenrod",
		cex = cx_scale, cex.lab = cx_scale, cex.axis = cx_scale, cex.main = cx_scale,
	  	xlab = names(mash[i]),
	  	main = "Dependent variable - Shares sequence")
	...
	for (i in 2 : length(mash))
	{
		   					...
		   					|| class(mash[,i]) == "matrix")
		{
			...
			abline(lm(mash$shares ~ mash[,i]))

# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# ...	look at residuals from one-variable linear fit			
# ...	-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

			fit <- lm(mash$shares ~ mash[,i], na.action = na.exclude)
			res <- resid(fit)
